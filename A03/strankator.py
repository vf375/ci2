...
print(str(dom1.find_all('iupacname')).replace('<iupacname>','').replace('</iupacname>, ','\n').replace('</iupacname>','')[1:-1]) #makes a string out of all cmpdname elements, then deletes unecessary xml bloat and replaces the ends of the element with a line break, 
